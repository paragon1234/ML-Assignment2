...
def _stem(doc, p_stemmer, en_stop, return_tokens, stem=True):
...
def getStemmedDocuments(docs, return_tokens=True, stem=True):
    """
        Args:
    ...
    if isinstance(docs, list):
        output_docs = []
        for item in docs:
            output_docs.append(_stem(item, p_stemmer, en_stop, return_tokens, stem))
        return output_docs
    else:
        return _stem(docs, p_stemmer, en_stop, return_tokens, stem)
